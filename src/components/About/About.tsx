...
import { GoArrowUpRight, GoStack } from "react-icons/go";
...
const About = () => {
   return (
      <main className="text-foreground">
         ...
         <p className="text-muted-foreground text-base mb-4 font-medium">
            Atualmente utilizo o ecossistema <TextAccent>React</TextAccent> para construir meus
            projetos, meu conhecimento consistente em{" "}
            <TextAccent>Vite</TextAccent> <TextAccent>Next.js</TextAccent> <TextAccent>Talwind</TextAccent> <TextAccent>Typescript</TextAccent> <TextAccent>Node.js</TextAccent> e pretendo
            aprofundar meu conhecimento em <TextAccent>Java</TextAccent> para desenvolver aplicações back-end.
         </p>
         <WorksExperience />
      </main>
   );
            ...
            {arrWorks.map((work) => (
               <div className="flex flex-col sm:flex-row gap-2 justify-between sm:items-center" key={work.company}>
                  <div className="flex gap-2 flex-col sm:flex-row">
                     ...
                     <span>
                        <h3>{work.company}</h3>
                        <p className="text-[12px] sm:text-sm text-muted-foreground font-medium">{work.role}</p>
                     </span>
                  </div>
                  <div>

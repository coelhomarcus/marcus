...
Faz um tempo que eu tô de olho na linguagem GO, e finalmente tomei coragem p/ estudar!

Aqui estão todas as anotações que fiz durante os estudos:
...
Função isEven para fixar conhecimento
```
package main
...
De forma resumida, em Value Types, precisamos usar ponteiros caso queiramos alterar seus valores por meio de funções. Já em Reference Types, não precisamos nos preocupar com isso, pois, mesmo que seja criada uma cópia ao passá-los em uma função, essa cópia aponta para o mesmo endereço, então a modificação ocorre normalmente.

### Map

Basicamente, Map é como se fosse um objeto do JavaScript, mas com algumas peculiaridades: as chaves devem ser todas do mesmo tipo, e os valores também devem ser todos do mesmo tipo.
...
Código que fiz para um desafio. Ainda não está otimizado, mas estou aprendendo.

```
func main() {
	// os.Args retorna uma slice de string com os argumentos
	// que passamos na execução: go run main.go arquivo.txt
	// fileNames[0] = main.go e fileNames[1] = arquivo.txt
	fileNames := os.Args

	// Aqui abrimos o arquivo cujo nome foi passado como argumento
	// textFile recebe o conteúdo (ou melhor, uma referência ao arquivo aberto)
	textFile, _ := os.Open(fileNames[1])

	// Criamos uma slice de bytes com tamanho 9999 para armazenar o conteúdo lido do arquivo
	byteSlice := make([]byte, 9999)

	// Usamos o método Read para preencher a byteSlice com os bytes lidos do arquivo
	textFile.Read(byteSlice)

	// Transformamos os bytes em string e printamos o conteúdo do arquivo no terminal

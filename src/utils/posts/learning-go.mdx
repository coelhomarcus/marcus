...
**Executáveis** `package main`: Geram programas que podem ser rodados. Precisam ter uma função main e são usados para criar o executável do projeto.

**Reutilizáveis** (qualquer outro nome): Servem como bibliotecas e podem ser importados em outros pacotes, mas não geram executável sozinhos.
...
Para criar um pacote, basta criar uma pasta com o nome desejado, por exemplo `alfabeto`. Dentro dela, coloque os arquivos Go: `a.go` `b.go`

Em todos os arquivos dentro dessa pasta, a primeira linha deve declarar o mesmo pacote: `package alfabeto`

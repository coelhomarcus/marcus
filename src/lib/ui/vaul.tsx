    ...
    <Drawer.Root direction="right" open={isOpen} onOpenChange={setIsOpen}>
      <Drawer.Trigger
        render={(props) => (
          ...
          >
            <ViewVerticalIcon className="w-5 h-5 hidden md:block text-muted-foreground" />
            <svg
      ...
      >
        {item.download ? (
          ...
          >
            ...
            <ArrowTopRightIcon className="w-5 h-5 text-muted-foreground group-hover:text-foreground transition-colors" />
          </a>
        )}
      </button>
    );
